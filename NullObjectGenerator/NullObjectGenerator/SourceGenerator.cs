...
";            
        ...
        private string GenerateClassForInherits(ISymbol symbol, ClassData data)
        {
            ...
            foreach (var iDataInterface in data.interfaces)
            {
                ...
                var nameSpace = iDataInterface.ContainingNamespace.ToDisplayString();
                allUsings.Add(nameSpace);
        ...
        private string GenerateClassForInterface(ISymbol symbol, InterfaceData data)
        {
            ...
            // usingディレクティブを取得
            IEnumerable<string> usings =  root.Usings.Select(usingDirective => usingDirective.Name.ToString()).ToList();
            string nameSpace = symbol.ContainingNamespace.ToDisplayString();
            var usingsAddedSelfnameSpace = usings.Append(nameSpace);
            
            string[] interfaces = new[] { data.interfaceDeclarationSyntax.Identifier.ValueText };
            
            return GenerateClass(symbol, data , usingsAddedSelfnameSpace , interfaces);
        }
        ...
        private string GenerateClass(ISymbol symbol, IImplementationData data , IEnumerable<string> usings , IEnumerable<string> interfaces)
        {
            ...
            var thisNameSpace = symbol.ContainingNamespace.ToDisplayString();

            foreach (var ns in usings)
            {
                if(thisNameSpace == ns) continue;
                sb.Append($@"using {ns};
");
            }
...
{{";
    ...
    /// <summary>
    /// This class is NullObject generated by NullObjectGenerator.
    /// </summary>
...
";
            ...
            sb.Append($@"
            {classAccessiblity} {symbol.Name}AsNullObj(){{ }}
    ");
            ...
            foreach (var method in data.Methods)
            {
                ...
                sb.Append($@"
            {accessiblity} {method.ReturnType} {method.Name}(");
